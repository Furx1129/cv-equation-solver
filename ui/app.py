...
def process(
    ...
    if image is None:
        return "", "", "", [], None, None, "", None, "请上传一张算式图片"
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            if mode == "auto":
                ...
                if recognition is None:
                    return "", "", "", [], None, None, "", None, f"识别失败：{decision.router_reason}"
            else:
                recognition = recognize_image(image_path=tmp_path, backend=mode)
        except Exception as exc:
            return "", "", "", [], None, None, "", None, f"识别异常：{exc}"
        ...
        if mode == "auto":
            ...
            f = analysis.features
            structure_text = (
                f"**路由决策**: {analysis.route_hint} (置信度 {analysis.route_confidence:.3f})\n\n"
                f"**路由理由**: {decision.router_reason}\n\n"
                f"**候选分数**:\n"
            )
            for candidate in decision.candidates:
                structure_text += f"- {candidate.category}: score={candidate.score:.3f} (reject: {candidate.reject_stage})\n"
            structure_text += (
                f"\n**图像特征**:\n"
...
with gr.Blocks(title="CV Equation Solver") as app:
    ...
    run_btn.click(
        ...
        outputs=[
            ...
            binary_img, segments_img, structure_md, warnings_out, status,
        ],
    )
